...
#| message: false
#| warning: false
t <- "../inventories/data/raw_data/harmonized_data_mbaiki_controls_v2.csv" %>% 
  read_harmonized() %>% 
  left_join(read_tsv("../species/outputs/taxonomy_v6.tsv") %>% 
              select(contains("cleaned") | "raw_taxonomy") %>% 
              unique())
...
#| message: false
#| warning: false
t <- "../inventories/data/raw_data/harmonized_data_mbaiki_v1.csv" %>% 
  read_harmonized() %>% 
  left_join(read_tsv("../species/outputs/taxonomy_v6.tsv") %>% 
              select(contains("cleaned") | "raw_taxonomy") %>% 
              unique())
t %>% 
  rename_all(tolower) %>% 
  group_by(year, plot) %>% 
  summarise(mean_log = exp(mean(log(diameter), na.rm = TRUE)),
            median_log = exp(median(log(diameter), na.rm = TRUE)),
            sd_log = sd(log(diameter), na.rm = TRUE)) %>% 
  gather(variable, value, -year, -plot) %>% 
  ggplot(aes(year, value, col = as.character(plot))) +
  geom_line() +
  theme_bw() +
