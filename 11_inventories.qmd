...
#| message: false
#| warning: false
#| fig-cap: "Subplots 15 basal area comparisons with total basal are of plot 15 through time."
list.files("data/harmonized_datasets_ss", 
                      full.names = TRUE, pattern = "paracou") %>% 
  lapply(read_csv, 
         col_types = cols(TreeFieldNum = col_character(),
  ...
  xlab(expression("Plot 15 Basal area of tree " >=10 ~"cm dbh ["~m^{2}~ha^{-~1}~"]")) +
  ylab(expression("Subplot of Plot 15 Basal area of tree " >=10 ~"cm dbh ["~m^{2}~ha^{-~1}~"]"))
```
...
#| message: false
#| warning: false
#| fig-cap: "Mean of non-missing subplots 15 basal area comparisons with total basal are of plot 15 through time."
list.files("data/harmonized_datasets_ss", 
                      full.names = TRUE, pattern = "paracou") %>% 
  lapply(read_csv, 
         col_types = cols(TreeFieldNum = col_character(),
 ...
 ungroup() %>% 
  ...
  facet_wrap(~ variable, scales = "free") +
  ggtitle("Paracou Plot 15") +
  ylab(expression("Plot 15 Basal area of tree " >=10 ~"cm dbh ["~m^{2}~ha^{-~1}~"]")) +
  xlab(expression("Mean of non-missing Subplot of Plot 15 Basal area of tree ["~m^{2}~ha^{-~1}~"]"))
```
...
#| message: false
#| warning: false
#| fig-cap: "Subplots 19 basal area comparisons with total basal are of plot 15 through time."
read_csv("data/harmonized_datasets_ss/output_MISIONES.csv") %>% 
  ...
  facet_wrap(~ variable, scales = "free") +
  ggtitle("Missiones Plot 19") +
  xlab(expression("Plot 19 Basal area of tree " >=10 ~"cm dbh ["~m^{2}~ha^{-~1}~"]")) +
  ylab(expression("Subplot of Plot 19 Basal area of tree " >=10 ~"cm dbh ["~m^{2}~ha^{-~1}~"]"))
```
...
#| message: false
#| warning: false
#| fig-cap: "Mean of non-missing subplots 19 basal area comparisons with total basal are of plot 15 through time."
read_csv("data/harmonized_datasets_ss/output_MISIONES.csv") %>% 
  filter(Plot == 19, Year != 2018) %>% 
  filter(LifeStatus) %>% 
  filter(!(Family %in% c("muerto", "ND"))) %>% 
  ...
  facet_wrap(~ variable, scales = "free") +
  ggtitle("Missiones Plot 19") +
  ylab(expression("Plot 19 Basal area of tree " >=10 ~"cm dbh ["~m^{2}~ha^{-~1}~"]")) +
  xlab(expression("Mean of non-missing Subplot of Plot 19 Basal area of tree ["~m^{2}~ha^{-~1}~"]"))
```
...
#| message: false
#| warning: false
#| fig-cap: "Paracou and Missiones basal area trajectories relative to predisturbance."
read_tsv("outputs/gathered.tsv") %>% 
...
#| message: false
#| warning: false
#| fig-cap: "Paracou and Missiones basal area trajectories relative to control."
read_tsv("outputs/gathered.tsv") %>% 
  group_by(site, year, rel_year, delta_ba, ba_pre, plot) %>% 
  summarise(ba = sum(ba)) %>% 
  left_join(
