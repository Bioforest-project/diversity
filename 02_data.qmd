...
The raw information needed would be number of individual per hectare, per site, per census/year, per plot, per species, and per cohort/diameter class with maybe an information on the plot area to later weigh the information. For instance using the dummy data:
...
read_tsv("data/dummy_data.tsv") %>% 
  ...
  mutate(dbh_class = ifelse(dbh < 30, 10, 30)) %>% 
  mutate(dbh_class = ifelse(dbh < 80, dbh_class, 80)) %>% 
  group_by(Forest, Plot, PlotArea, CensusYear, Family, Genus, Species, dbh_class) %>% 
  summarise(density = n()/unique(PlotArea)) %>% 
  rename(forest = Forest, area = PlotArea, year = CensusYear, family = Family, genus = Genus, species = Species) %>% 
  DT::datatable()
```
