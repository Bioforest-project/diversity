data {
  ...
  int<lower=0> n_site ;
  int<lower=0> n_plot ;
  vector[n_rec] agb_rec ;
  vector[n_old] agb_old ;
  vector[n_pre] agb_pre ;
  vector[n_rec] time ;
...
parameters {
  real<lower=0, upper=1000> mu_theta0 ; // starting point
  real<lower=0> sigma_theta0 ;
  vector<lower=0, upper=1000>[n_plot] theta0_p ;
  real<lower=0, upper=0.5> mu_lambda ; // recovery rate
  real<lower=0> sigma_lambda ;
  vector<lower=0, upper=0.5>[n_plot] lambda_p ;
  real<lower=0, upper=1000> mu_thetaInf ; // ending point
  real<lower=0> sigma_thetaInf ;
  vector<lower=0, upper=1000>[n_site] thetaInf_s ;
...
model {
  ...
  agb_old ~ lognormal(log(mu_old), sigma_old) ;
  agb_pre ~ lognormal(log(mu_pre), sigma_pre) ;
  agb_rec ~ lognormal(log(mu_rec), sigma_rec) ;
...
generated quantities {
  vector[n_rec] pred_agb_rec ;
  for(l in 1:n_rec){
    pred_agb_rec[l] = lognormal_rng(theta0_p[plot_rec[l]] * (1 - exp(-lambda_p[plot_rec[l]] * time[l])), sigma_rec) ;
  }
}
